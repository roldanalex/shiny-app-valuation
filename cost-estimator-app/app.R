...
# Referenced so rsconnect bundles the PDF-report toolchain (these are used
# from report/estimate_report.qmd, which rsconnect's scanner doesn't parse)
...
# Allow ZIP uploads up to the advertised 50MB limit
...
# Source design tokens, helpers, and UI/server modules
source("modules/theme.R")
source("modules/app_helpers.R")
source("modules/pdf_report.R")
...
ui <- page_navbar(
  ...
  header = tags$head(
    ...
    tags$style(HTML(css_root_vars())),
    tags$link(rel = "stylesheet", href = "custom.css")
  ),
  theme = app_theme(),
  footer = tags$footer(
    class = "app-footer py-2 px-3 mt-auto",
    tags$span(paste0("Shiny Cost Estimator v", APP_VERSION)),
  ...
  nav_panel(
    ...
    layout_sidebar(
      ...
      # Results area
      analysisResultsUI("analyze_results")
    )
  ),
  ...
  nav_spacer(),
  nav_item(
    actionButton("btn_how_to_use", label = tagList(icon("circle-question"), "How to Use"),
...
server <- function(input, output, session) {
  ...
  output$demo_stats <- renderUI({
    ...
    layout_column_wrap(
      ...
      div(class = "demo-card-stat",
        div(class = "stat-val", paste0("$", format(e$cost_usd, big.mark = ","))),
      ...
      div(class = "demo-card-stat",
        div(class = "stat-val",
            paste0("$", format(e$estimate_range$low, big.mark = ","),
                   " – $", format(e$estimate_range$high, big.mark = ","))),
        div(class = "stat-lbl", "Estimate Range (±30%)")
      )
    )
  })
  ...
  observeEvent(input$btn_how_to_use, {
    showModal(modalDialog(
      includeMarkdown("markdown/app_manual.md"),
  ...
  observeEvent(input$btn_release_notes, {
    showModal(modalDialog(
      ...
      title = paste0("Release Notes - v", APP_VERSION),
      size = "l",
      easyClose = TRUE,
      footer = modalButton("Close")
    ))
  ...
  help_content <- list(
    ...
    maxsched = list(
      ...
      body = tagList(
        ...
        p("The premium grows smoothly with the amount of compression",
          " (following COCOMO II's schedule-compression driver) and is",
          " capped at +100%. Halving the natural schedule adds roughly",
          " +40% to cost; deeper compression costs progressively more."),
        p("If the compressed schedule would need more people than your",
          " team cap allows, the app flags the plan as not feasible."),
    ...
    if (method == "local") {
      ...
      tryCatch({
        withProgress(message = "Analyzing repository...", value = 0, {
          ...
          results$analyze <- list(
            ...
            source_label = basename(normalizePath(input$local_path))
          )
        })
      ...
      tryCatch({
        withProgress(message = "Analyzing ZIP contents...", value = 0, {
          ...
          results$analyze <- list(
            lang_summary = analysis,
            source_label = tools::file_path_sans_ext(basename(input$zip_file$name))
          )
        })
      ...
      # ── Manual entry ──
      # Sanitize: blank/NA/negative/non-numeric inputs become 0, then drop zeros
      language_mix <- build_language_mix(list(
        "R"          = input$manual_r,
  ...
  analysis_handles <- analysisResultsServer("analyze_results", analyze_data, analyze_params)
  ...
  exportServer("export", results, analysis_handles, list(parent_session = session))
  ...
  observe({
    ...
    if (!is.null(query$mode) && identical(query$mode, "manual")) {
      # All numeric params are bounds-checked; invalid values are ignored
      r_val   <- parse_num_param(query$r)
      py_val  <- parse_num_param(query$py)
      js_val  <- parse_num_param(query$js)
      sql_val <- parse_num_param(query$sql)
      team_val <- parse_num_param(query$team, lo = 1, hi = 5)

      if (!is.null(r_val))   updateNumericInput(session, "manual_r",      value = r_val)
      if (!is.null(py_val))  updateNumericInput(session, "manual_python", value = py_val)
      if (!is.null(js_val))  updateNumericInput(session, "manual_js",     value = js_val)
      if (!is.null(sql_val)) updateNumericInput(session, "manual_sql",    value = sql_val)
      if (!is.null(query$complexity) &&
          query$complexity %in% c("low", "medium", "high"))
        updateSelectInput(session, "analyze_complexity", selected = query$complexity)
      if (!is.null(team_val))
        updateSliderInput(session, "analyze_team_exp", value = round(team_val))
